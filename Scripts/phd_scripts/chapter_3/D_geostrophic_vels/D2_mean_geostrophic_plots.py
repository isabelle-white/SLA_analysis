...
D2_mean_geostrophic_plots.py

same as D1 but calculates the mean ug and vg over the given time period 
...
gyre_name = 'full' #ross #weddell #kerguelen # full
...
gyre_fig_dir = fig_dir + gyre_name + '/'
...
# data_t = to_plot[time_key]

#calculating variable means for mean flow plot
dot_stack = np.stack([var['dot'] for var in to_plot], axis=0)
dot_mean = np.nanmean(dot_stack, axis=0)

ug_stack = np.stack([var['ug'] for var in to_plot], axis=0)
ug_mean = np.nanmean(ug_stack, axis=0)

vg_stack = np.stack([var['vg'] for var in to_plot], axis=0)
vg_mean = np.nanmean(vg_stack, axis=0)
...
dot_plot = plt.pcolormesh(mean_data['lon'], mean_data['lat'], mean_data['dot_mean'], cmap='RdBu_r')
if gyre_name == 'full':
    quiver = plt.quiver(
        mean_data['lon'][::quiver_step],
        mean_data['lat'][::quiver_step],
        mean_data['ug_mean'][::quiver_step, ::quiver_step],
        mean_data['vg_mean'][::quiver_step, ::quiver_step],
...
ax.set_title(f'Mean geostrophic velocities')
ax.set_xlabel('Longitude')
ax.set_ylabel('Latitude')
# plt.tight_layout()

file_save_name = f'{gyre_name}_quiver_mean_plot_rectangular.png'
