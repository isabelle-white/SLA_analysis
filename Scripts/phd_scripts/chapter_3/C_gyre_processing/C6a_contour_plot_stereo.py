"""
C6a_contour_plot_stereo.py

-Loads data from  C1 (verts.npz file)
...
end_time = '2024-12-01'
white_contours = True #True # False
...
for i, t in enumerate(ts_params_ds.time.values):
    ...
    to_plot.append({
        ...
        'mdt_verts_max': mean_verts['mdt_contour_max_verts']
    })

mdt_contour_verts = to_plot[0]['mdt_verts_max']
...
if gyre_name == 'ross':
    ...
    st.spstere_plot_gyre_contours(ax, m_sec, wedge_patch,
                                   to_plot, gyre_name = gyre_name, lat_mask_max = lat_mask_max, lon_sigma = lon_sigma, lat_sigma = lat_sigma,
                                  plot_mode=None, variable=variable, white_contours = white_contours)

    if mdt_contour_verts is not None:
        st.spstere_mean_contour_plot(ax, m_sec, wedge_patch, mdt_contour_verts, gyre_name = gyre_name, lat_mask_max = lat_mask_max, lon_sigma = lon_sigma, lat_sigma = lat_sigma,
                              color='black', linewidth=2, label='MDT gyre outline')

    if white_contours == True:
        save_fig_name = f'{tag}_contours_stereo_white.png'
        plt.savefig(gyre_fig_dir + save_fig_name, dpi=300, bbox_inches='tight')
    else:
        save_fig_name = f'{tag}_contours_stereo.png'
        plt.savefig(gyre_fig_dir + save_fig_name, dpi=300, bbox_inches='tight')
