"""
C7b_contour_params_plot_smooth.py
...
dot_mean = ts_params_ds['dot_mean'].rolling(time = 3, center = True, min_periods = 2).mean()
area_km2 = ts_params_ds['area_km2'].rolling(time = 3, center = True, min_periods = 2).mean()
contour_level = ts_params_ds['level'].rolling(time = 3, center = True, min_periods = 2).mean()
strength_basic = ts_strength_ds['strength_basic'].rolling(time = 3, center = True, min_periods = 2).mean()
strength_norm = ts_strength_ds['strength_normalised'].rolling(time = 3, center = True, min_periods = 2).mean()
centre_lat = ts_center_centroid_ds['centre_lat'].rolling(time = 3, center = True, min_periods = 2).mean()
centroid_lat = ts_center_centroid_ds['centroid_lat'].rolling(time = 3, center = True, min_periods = 2).mean()
...
for i, var in enumerate(var_to_plot):
    axes[i].plot(time, var, linewidth = 1.5, color = colours_to_plot[i] )
...
save_fig_name = tag + '_params_timeseries_smooth.png'
